...
Node::Node (ORB_SLAM2::System* pSLAM, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  if (publish_pose_param_) {
    pose_publisher_1 = node_handle_.advertise<geometry_msgs::PoseStamped> (name_of_node_+"/pose", 1);
    pose_publisher_2 = node_handle_.advertise<geometry_msgs::PoseWithCovarianceStamped> (name_of_node_+"/pose_with_covariance", 1);
  }
}
...
void Node::PublishPositionAsPoseStamped (cv::Mat position) {
  ...
  geometry_msgs::PoseWithCovarianceStamped pose_2_msg;
  tf::poseStampedTFToMsg (grasp_tf_pose, pose_msg);
   pose_2_msg.header.stamp = pose_msg.header.stamp;
  for ( auto & x : pose_2_msg.pose.covariance ) {
               x = 0.0;
  }
  pose_2_msg.pose.covariance[0] = 1;
  pose_2_msg.pose.covariance[7] = 1;
  pose_2_msg.pose.covariance[14] = 1;
  pose_2_msg.pose.covariance[21] = 1;
  pose_2_msg.pose.covariance[28] = 1;
  pose_2_msg.pose.covariance[35] = 1;
  pose_2_msg.pose.pose=pose_msg.pose;
  pose_publisher_1.publish(pose_msg);
  pose_publisher_2.publish(pose_2_msg);
}
